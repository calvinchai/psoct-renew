...
if strcmp(filetype, 'mat')
    switch lower(modality)
        case 'retardance'; modality_mat = 'Ret';
        case 'orientation'; modality_mat = 'Orien';
        case 'birefringence'; modality_mat = 'Bi';
        otherwise; modality_mat = modality;
    end
end
...
for s = 1:size(sliceidx,2)
    ...
    if strcmpi(modality,'orientation')
        Mosaic  =zeros(MXL,MYL,4,'like',sample);
        M       =zeros(MXL,MYL,4,'like',sample);
    else 
        Mosaic  =zeros(MXL,MYL,'like',sample);
        M       =zeros(MXL,MYL,'like',sample);
    end
    ...
    %         fprintf('row %d\n',jj);
            if MapIndex(jj,ii)>0 && ~isnan(X(jj,ii))
                ...
                if strcmpi(modality,'orientation')
                    ...
                    Mosaic(row,columns,:)=Mosaic(row,columns,:)+SLO_OC.*repmat(RampOrig,1,1,4);
                    Masque(row,columns) = Masque(row,columns)+RampOrig;
                else % mus, aip, mip, retardance
                    Mosaic(row,columns) = Mosaic(row,columns)+I.*RampOrig;
                    Masque(row,columns) = Masque(row,columns)+RampOrig;
                end
            end
            M=M+Mosaic;
    ...
    %             a = [Ma(st,tt,1) Ma(st,tt,3);Ma(st,tt,2) Ma(st,tt,4)];
    %             [V, D] = eig(a);
    ...
    %     toc
        ...
        [V,~]= eig2(a_x);
        Data2 = atan(V(:,2,2)./V(:,1,2));
        Data2 = reshape(Data2, size(M,[1 2]));
    %     fprintf('Tensor calculation complete\n');
        ...
        MosaicFinal=rot90(O,-1);
        map3D=ones([size(MosaicFinal) 3]);
        map3D2=ones([size(MosaicFinal) 3]);
        O = (MosaicFinal+90)/180;
        ...
        if FovAdjustFlag == 1
            RetSliceTiff = [outdir filesep 'Retardance_adjustFOV__slice' sprintf('%03i',sliceid_out) '.tiff'];
            AipSliceTiff = [outdir filesep 'AIP_adjustFOV_slice' sprintf('%03i',sliceid_out) '.tiff'];
        else
    ...
    %         imrsz = imresize(inimg,0.3,'bilinear');
    %         % figure; imshow(imrsz); title('resized image');
    %         imwrite(imrsz,[output outputFolder 'resized-Orientation2_slice' sprintf('%03i',sliceid_out) '.tiff'],'compression','none');
    ...
    %         imrsz = imresize(inimg,0.3,'bilinear');
    %         % figure; imshow(imrsz); title('resized image');
    %         imwrite(imrsz,[output outputFolder 'resized-Orientation1_slice' sprintf('%03i',sliceid_out) '.tiff'],'compression','none');
