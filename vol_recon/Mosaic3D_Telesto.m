...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% ADDING CLIPPING
XPixClip    = 20; %clipped from both side . each are 10 px
...
sizerow = 200-XPixClip;                sizecol = 350-YPixClip;
...
%             fprintf('row %d / %d\n',jj,tabrow)
            ...
            if MapIndex(jj,ii)>0 && ~isnan(X(jj,ii))
                ...
                switch filetype
                    case 'mat'
                        ftile = [indir_curr filesep sprintf('dBI3D_%03i.mat',currtile)];
                        S = whos('-file',ftile);
                        load(ftile);
                        Imag = eval(S.name);
                        Imag = flip(rot90(Imag,-1),2); % just for this medulla cass
                    case 'nifti'
...
%                 Imag =  Imag(:,:,end:-1:1); 
    ...
    if 1==1 % approve this flag when 'MosaicFinal' is smaller than 10 G 
        disp(' - Saving mosaic...');
        fout = [outdir filesep modality sprintf('_slice%03i_mean-xy_10um-iso.mat',sliceid_out)];
        fprintf(' %s \n',fout);
        aatic=tic;
...
%         view3D(Id);
fout = [outdir filesep modality sprintf('_slice%03i_mean-xy_20um-iso.mat',sliceid_out)];
