...
% Define file path
file_path = '/space/megaera/2/users/kchai/000052/tile_data/';
outdir = '/space/megaera/1/users/kchai/code/psoct-renew/reprocessed/';
...
for tilenum = all_tissues
    ...
    for i = 1:nx
        for j = 1:ny
            ...
            if ~isempty(slopes)
                biref(i,j) = prctile(slopes, 80);  % Use 80th percentile
            else
                biref(i,j) = 0;
            end
    ...
    % Define file paths
    real_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_biref.nii', mosaicnum, tilenum));
    real_name_v = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_biref_v.nii', mosaicnum, tilenum));

    replace_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_processed_biref.nii', mosaicnum, tilenum));
