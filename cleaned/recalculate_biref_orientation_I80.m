...
disp(mosaicnum)
% Loop over all tiles (you should define `all_tissues` beforehand, e.g., all_tissues = 1:588;)

for tilenum = all_tissues
    tilenum
    % Construct full filename
    filename = [file_path, sprintf(['mosaic_%03d_image_%04d_processed_cropped_focus.nii'], mosaicnum, tilenum)];
    % disp(file_path)
...
% %%%%%%%%
    ...
    %             denom = sum((x - x_mean).^2);
    %             slope = abs(numer / denom);
    %             slopes_ref(end+1) = slope;
    % 
    % 
    %         end
    %         if ~isempty(slopes)
    %             biref(i,j) = prctile(slopes, 95);  % 95th percentile
    %             biref_ref(i,j) = prctile(slopes_ref, 95);  % 95th percentile
    %         else
    %             biref(i,j) = 0;
    %             biref_ref(i,j)=0;
    %         end
...
%%%%%%%%%
    ...
    % Define file paths
    real_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_orientation.nii', mosaicnum, tilenum));
    real_name_ref = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_orientation_shallow.nii', mosaicnum, tilenum));
    % replace_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_processed_orientation.nii', mosaicnum, tilenum));
    ...
    % Define file paths
    real_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_biref_v1_otlr.nii', mosaicnum, tilenum));
    ref_name = fullfile(file_path, sprintf('mosaic_%03d_image_%04d_reprocessed_biref_v2.nii', mosaicnum, tilenum));
    ...
    % biref_single = single(biref_ref);  % ensure single precision for NIfTI
    % niftiwrite(biref_single, ref_name, 'Compressed', false);
